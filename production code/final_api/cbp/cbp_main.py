...
import age_calculator,Service_imperson
...
cbp_router=APIRouter(prefix="/cbp",tags=["CBP"])

access_denied="ACCESS DENIED: You don't have permission to access this resource."
table_not_found_message="Table not found: The specified table does not exist in BigQuery."



@cbp_router.get("/allCBP",response_model=List[all_cbp_schema.All_cbps])
def get_all_cbps():
    """Display all CBP's"""
    try:
        sql=f"""
...
@cbp_router.get("/search_cbp/:cbp_id",response_model=fetch_cbp_schema.CBP)
...
@cbp_router.put("/update_cbp",response_model=str)
