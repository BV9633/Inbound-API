...
FALLBACK_MODELS = [
    "gemini-1.5-flash-8b",        # Very fast, 8k context, good for tabular
    "gemini-2.0-flash-lite-02-05", # Newer light model, good for this task
    "gemini-1.5-flash",           # Balanced, 1M context
    "gemini-2.0-flash-001",       # Older flash model - might be slightly slower
]
...
def _call_gemini_blocking(metadata: Dict[str, Any]) -> Dict[str, Any]:
    ...
    system_instruction = """You are a strict Invoice Data Extractor for an Enterprise ERP system.
    ...
    1.  The document's primary title or header (found prominently within the first 15 rows) explicitly contains "INVOICE" or "COMMERCIAL INVOICE".
    2.  A valid, non-empty "INVOICE NUMBER" value can be extracted.
    ...
    **STRICT EXCLUSION RULES:**
    ...
    CANONICAL FIELDS (9 fields - use these exact names):
    supplier_name, supplier_location, invoice_number, invoice_date, currency, Incoterm, commercial_invoice_value, HAWB_number, MAWB_number

    LINE ITEMS (7 fields - map source columns to these):
    1. ASN → Look for: "ASN", "ASN#", "ASN No", "ASN No."
    2. part_number → Look for: "PART NUMBER", "M# PARTE / PART NUMBER", "Part No.", "Part No. / Part name", "Customer Part No.", "Jabil PIN#", "Customer PIN #", "PIN#", "SKU", "Item No", "Material", "ITEM EBSQ"
    3. PO → Look for: "PO", "PO#", "PO Number", "SO or PO", "Ref.P/O No.", "S/O No.", "Sales Order No.", "Purchase Order", "Order No"
    4. Quantity → Look for: "QTY", "Q'TY", "QTY(PCS)", "Qty to Ship", "Quantity", "UNIT", "PCS", "Units" (must be a number)
    5. Total_Value → Look for: "TOTAL", "Value", "Amount", "Amount USD", "TOTAL AMOUNT", "Line Total", "Extended Price", "Ext Price" (must be a number)
    6. unit_price → Look for: "UNIT PRICE", "Unit Price USD", "Price", "Rate", "Unit Cost" (must be a number)
    7. country_of_origin → Look for: "COUNTRY OF ORIGIN", "Country Of Origin", "COO", "Origin", "Made In"

    === STRICT OUTPUT RULES ===
    1. Return JSON ONLY.
    2. OUTPUT ONLY these 7 line item fields.
    3. If a field's value cannot be found or is empty, OMIT that field entirely from the JSON output.
    4. Numbers should be actual numbers (e.g., 10, 5.50), not strings or currencies.
    5. Ensure the 'document_type' field is set to "INVOICE" or "COMMERCIAL_INVOICE" if is_invoice=true.
    """
