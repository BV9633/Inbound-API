...
from schemas import all_waybills_schema,fetch_waybill_schema,cancel_schema
import age_calculator
import fetch_waybill
import pdf_extractor
import timestamp
import update_waybill
...
@app.get("/search_waybill/:waybill_id",response_model=fetch_waybill_schema.Waybill)
...
def cancel_update(payload:cancel_schema.Cancel_waybill):
    try:
        ...
        if job.num_dml_affected_rows==0:
            raise HTTPException(status_code=404,detail=f"waybill id {str(payload.invoice_id)} not found")
        else:
            return "updation changes are cancelled"
